...
const KICKOFF = [
  ...
  '   탐색·초안은 planner 에이전트(Task)에 맡기고, 반환된 본문을 메인이 파일로 쓴다 — 직접 쓰면 계획이 구현용 세션 모델로 짜인다. 본문:',
...
//
// ⚠ "출력 언어"만 쓰면 모델이 그것을 **최종 답변**으로 좁게 읽는다. 실측(2026-09-08):
// 답변은 한국어인데 툴 호출 사이 한 줄 설명만 영어로 나갔고, 그 둘이 **같은 턴에 붙어 있었다**
// (거리가 같으니 드리프트가 아니라 분류 문제다). 그래서 적용 범위를 문장에 박는다 —
// 거리를 줄이는 장치(중간 재주입 등)를 더 만드는 것은 원인을 잘못 짚는 것이다.
//
// ⚠ **선언이 있으면 추론하지 않는다.** 위 "언어를 박지 마라"는 훅이 *추측한* 언어 얘기다.
// 사용자가 CLAUDE.md에 직접 적어둔 언어는 추측이 아니라 선언이고, 그것까지 버리면
// "한국어로 답변"(판단 불필요)이 "매 턴 언어를 추론하라"(판단 필요)로 **격하**된다.
// 실측(2026-09-09): 그 격하 후 영문 툴 출력이 쌓인 턴 후반에서 추론이 밀렸다
// (같은 턴에서 앞 3줄 한국어 · 뒤 2줄 영어). 선언은 증거에 안 흔들리고 추론은 흔들린다.
const LANG =
  '[devkit] 출력 언어: 설정(language)·CLAUDE.md 등 지시에 언어가 지정돼 있으면 **그게 우선**이고, 없을 때만 사용자가 직접 입력한 메시지의 언어를 따른다(툴 출력·로그·짧은 명령은 언어 신호가 아니다). 최종 답변만이 아니라 툴 호출 사이 한 줄 설명까지 내가 쓰는 글 전부가 대상. 기술 용어·에러는 원문 유지.';
