...
- **`/merge`** — 열린 PR을 스쿼시 머지하고 로컬을 그 뒤 상태로 맞춘다. `/ship`이 내보내는
  쪽이었고 받는 쪽이 없었다. 원격 브랜치 삭제·`main` 동기화까지 무프롬프트로 가되,
  **로컬 브랜치 삭제만 확인받는다**: 스쿼시 머지 후에는 `git branch -d`가 반드시 실패하고
  (스쿼시가 새 커밋을 만들어 원 브랜치가 조상이 아니게 된다) 남는 선택지가 `-D`뿐인데,
  그건 확인이 필요한 명령이다. 더러운 작업 트리에서는 시작 전에 멈춘다.
  CI 실패는 게이트가 아니지만 결론 줄에 반드시 적는다.

### 고침 (0.20.2)

- **`dep-guard`가 bare install을 차단하던 것.** `2>&1`의 `2`가 패키지 이름으로 읽혀
  `pnpm i 2>&1 | tail -5` 같은 **기존 의존성 복원**이 "새 의존성 추가"로 막혔다.
  `> /dev/null`은 통과하는데 `2>&1`만 걸려서 눈에 안 띄었다 — `>`는 패키지 문자셋 밖이지만
  `2`는 `\w`다. 이제 매칭 전에 리다이렉션을 뗀다. 실측 오탐 4건이 전부 사라지고
  진짜 추가 4건은 그대로 차단된다. ⚠ heredoc 본문은 일부러 안 뗀다 — 떼면
  `cat <<EOF | sh` 안에 설치 명령을 숨길 수 있고, 오탐보다 미탐이 훨씬 비싸다.
- **의존성 규칙이 한쪽 면만 말하던 것.** "추가는 승인 필요"만 있고 **우회에 대한 문장이
  없어서**, 모델에게는 게이트를 안 건드리는 설계가 가장 싼 길이었다(실사용 보고:
  "승인이 필요하니 라이브러리 없는 방향으로 만들겠습니다"). `dep-guard` 훅은 설치를
  실제로 시도해야 발동하므로 **이 실패 모드에서는 훅이 원리적으로 안 보인다** — 문장으로만
  막을 수 있다. RULES(SUMMARY 포함) · `architect` · `tdd-driver` · 훅 메시지 네 곳을
  양면으로 고쳤다: 승인 비용은 트레이드오프가 아니고, 우회했다면 그 대가를 보고에 적는다.
