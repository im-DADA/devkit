...
// ── B20: 툴 호출 사이의 한 줄 설명도 같은 언어 ────────────────────────
// 실측(2026-09-08 사용자 제보): 최종 답변은 한국어인데 툴 호출 사이 진행 설명만 영어로 나갔다.
//   "Plan is written. Here it is for approval before I implement."  ← 영어
//   "계획 요약 (docs/…/PLAN.md, Quick 트랙)"                          ← 한국어 (바로 다음 줄)
// **같은 턴에 붙어 있는 두 줄이라 컨텍스트 거리가 같다** — 드리프트로는 설명되지 않는다.
// 첫 툴 호출부터 이미 영어였다("I'll start by reading the feedback doc.").
// 원인은 "출력 언어"를 모델이 "사용자에게 주는 답변"으로 좁게 읽고, 진행 설명을 그 밖으로
// 분류한 것이다. 그래서 **적용 범위**를 문장에 박는다 — 거리를 줄이는 장치를 더 만들지 않는다.
test('B20: 툴 호출 사이 진행 설명에도 적용됨을 밝힌다', () => {
  const cwd = makeRoot();
  const line = langLine(run({ prompt: '이거 구현해줘', cwd }).stdout);
  assert.match(
    line, /툴 호출|진행 설명|한 줄 설명/,
    '툴 호출 사이의 설명도 대상임을 밝혀야 함 — "출력=최종 답변"으로 읽히면 preamble이 샌다',
  );
  assert.match(
    line, /전부|모든|모두/,
    '적용 범위가 "내가 쓰는 글 전부"임을 밝혀야 함',
  );
});

// ── B21: 사용자가 선언한 언어에 양보한다 ──────────────────────────────
// B4는 "훅이 **추측한** 언어를 박지 마라"였다(PLAN: `claude mcp add …` 한 줄에 English를
// 박아 문제를 유발한다). 그런데 그게 "언어 이름을 절대 쓰지 마라"로 일반화되면서,
// 사용자가 CLAUDE.md에 **직접 선언한** 언어까지 같이 버려졌다. 둘은 다르다 —
// 추측은 틀릴 수 있지만 선언은 틀릴 수가 없다.
//
// 실측(2026-09-09): devkit 없을 때는 전역 CLAUDE.md의 "한국어로 답변" 하나뿐이었고 안 샜다.
// devkit이 그것을 "매 턴 언어를 추론하라"로 바꾸고 **더 가까운 자리**에 두자,
// 영문 툴 출력이 쌓인 턴 후반에서 추론이 밀렸다(앞 3줄 한국어 · 뒤 2줄 영어).
// 선언은 증거에 안 흔들리고 추론은 흔들린다 — 그래서 선언이 있으면 그쪽에 양보한다.
test('B21: 지시에 언어가 지정돼 있으면 그것이 우선임을 밝힌다', () => {
  const cwd = makeRoot();
  const line = langLine(run({ prompt: '이거 구현해줘', cwd }).stdout);
  assert.match(line, /우선/, '지정된 언어가 우선임을 밝혀야 함');
  assert.match(
    line, /CLAUDE\.md|지시/,
    '어디에 지정된 것을 말하는지 밝혀야 함 — 안 밝히면 모델이 무엇을 볼지 모른다',
  );
  assert.match(
    line, /없|아니면|그 외/,
    '지정이 없을 때의 폴백(사용자 메시지 언어)이 남아 있어야 함',
  );
});

// ── B22: 설정(language)에 지정된 언어가 우선 ───────────────────────────
// 전역 CLAUDE.md를 없애면 "한국어로 답변" 선언이 사라진다. 선언은 규칙 텍스트가 아니라 사용자별
// settings `language`로 옮긴다(설치자에게 한국어를 강제하지 않는다). LANG 문장이 그 자리를 가리켜야 한다.
test('B22: 언어 지시가 설정에 지정된 언어를 우선한다고 밝힌다', () => {
  const cwd = makeRoot();
  const line = langLine(run({ prompt: '이거 구현해줘', cwd }).stdout);
  assert.match(line, /설정/, '설정(language) 우선이 없다 — 전역 CLAUDE.md 없이 언어 선언이 사라진다');
});
