...
// 부정 주장은 증상이고, 원인은 "대강 훑고 단정"이다. 출력만 막으면 같은 습관이
// 긍정 주장("이렇게 돌아간다")으로 새어나간다 — 그쪽은 사용자가 더 늦게 발견한다.
test('A5: 동작 단정에도 탐색 기준이 걸린다 (부정만 막으면 긍정으로 샌다)', () => {
  const rules = read('RULES.md');
  assert.match(rules, /경로를 안 따라가고 동작 단정 금지/, 'RULES.md: 경로 추적 규칙 없음');
  assert.match(rules, /진입점부터 실제 실행 지점까지 순서대로 말할 수 있나/, 'RULES.md: 검사법 없음');
  assert.match(rules, /호출부/, 'RULES.md: 정의↔호출부 양방향 확인 문장 없음');
  assert.match(rules, /부분만 봤으면 부분만 봤다고/, 'RULES.md: 부분 탐색 보고 규칙 없음');
  const summary = (rules.match(/<!-- SUMMARY:START -->([\s\S]*?)<!-- SUMMARY:END -->/) || [])[1];
