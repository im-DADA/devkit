...
- 사용자 언어로 간결히. 결론부터.
- ❌ 금지: 추측 답변, **부재 단정**("못 찾음"≠"없음" — 숨는 자리를 다 보기 전엔 "없다" 금지, 조회 가능하면 조회), 요청 안 한 코드·문서 생성(PDCA 산출물은 예외).
...
- ❌ **부재 증명 금지 — "못 찾음"과 "없음"은 다르다.** 몇 군데 훑고 안 나온 것은 *내 탐색이 좁았다*는 뜻이지 없다는 뜻이 아니다. 존재 주장은 한 곳만 보이면 서지만 **부재 주장은 숨을 수 있는 곳을 전부 봐야** 선다 — 필요한 근거의 양이 비대칭이다.
  - ⚠ 함정은 **"grep을 돌렸으니 추측이 아니다"** 이다. 그 grep이 준 근거는 세상이 아니라 **내 grep에 대한 것**이다. 위 "추측 금지"가 이 실패를 못 잡는 이유가 이것이라 따로 적는다.
  - **결과를 조회할 수 있으면 추론하지 말고 조회한다.** 마이그레이션·스키마·배포·cron은 전부 흔적이 남는다(`_prisma_migrations` 테이블, drizzle journal, `\dt`, 배포 로그). 코드를 읽어 "돌았을 것/안 돌았을 것"을 추정하는 것보다 **실행 결과를 직접 보는 쪽이 싸고 정확하다.**
  ...
  - 숨는 자리(Node/Next 기준): `instrumentation.ts` · `middleware.ts` · `next.config.ts` · package.json의 `postinstall`/`prebuild`/`start` · `Dockerfile`의 `CMD`/`ENTRYPOINT` · CI 워크플로 · 호스팅 대시보드(빌드·시작 명령, 스케줄러). **package.json scripts만 보고 단정하지 않는다.**
  - 실제 사례: package.json에 없다고 자동 마이그레이션이 안 돈다고 단정 → 실제로는 `src/instrumentation.ts`에 있었다.
  - 📢 **보고 형식이 규칙의 절반이다.** 부재를 말할 땐 "없다"가 아니라 **"어디를 봤는데 안 나왔다"**로 적는다 — 「`package.json`·`Dockerfile`·CI를 봤는데 마이그레이션 실행 지점이 안 보인다」. "없다"는 사용자가 반박할 근거를 안 주지만, 탐색 범위를 밝히면 **좁았다는 게 눈에 보여** 사용자가 즉시 교정할 수 있다. 확신이 없으면 단정하지 말고 **어디를 더 봐야 하는지 묻는다.**
