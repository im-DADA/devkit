...
- **턴 종료 시 typecheck·lint가 1회 돈다**(`stop-verify`). 스크립트가 없으면 조용히 넘기지 않고 세션당 한 번 알린다 — 검증되는 줄 알았는데 아무것도 안 도는 상태가 제일 나쁘다. **실행 실패(설정 오류·타임아웃)는 진단으로 세지 않는다.** 끄려면 `DEVKIT_VERIFY=off`.
