...
- **규칙을 추가·수정하면 같은 문서의 예시가 그 규칙을 위반하는지 확인한다.** 예시는 산문보다 강하게 복사되므로, 둘이 충돌하면 규칙이 아니라 예시가 이긴다. 스키마 예시·코드 스니펫·기본값이 대상이다.
...
- `output`은 **러너가 낸 출력을 그대로** 붙인 것이다. 위의 `✔`는 node:test가 찍은 문자지 사람이 붙이는 마커가 아니다 — pytest·go test는 그 러너의 출력 형식 그대로 쓴다.
...
| `uncited` | (그 명령을 돌렸는데) `output`의 인용이 그 출력에 있나 | 보고 |
