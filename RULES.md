...
- 🟡 **운영 DB 수정·쓰기·삭제**(INSERT·UPDATE·DELETE·DDL·마이그레이션 적용) → **사용자가 요청했을 때만.** 조회(SELECT·`\d`)는 자유.
  - "확인해봐"·"조사해줘"·"왜 이래?"는 쓰기 요청이 아니다. 조사하다 고칠 게 보이면 **SQL을 보여주고 묻는다** — 실행하고 보고하지 않는다.
  - 요청받아 쓸 때도 `BEGIN … COMMIT`으로 묶고 영향 행 수를 먼저 본다. 테스트용으로 바꾼 데이터는 끝나면 되돌린다.
  - 실측(2026-09-15): 최근 30일 자동 모드 분류기 거절 중 DB 관련이 가장 많았고, 대부분 운영 DB `UPDATE`·`DELETE`·`CREATE INDEX`·`migrate deploy`였다. 분류기가 막은 건 정상 동작이다 — `Bash(psql:*)` allow로 여는 대신 이 규칙으로 쓰기 시점을 사용자에게 둔다.
...
- 가드 훅은 **되돌릴 수 없는 것만 차단**(위험 명령·`.env` 덮어쓰기·`.git`·시크릿·리뷰 없는 REPORT)하고, 확인만 필요한 것(새 의존성·lockfile·`node_modules`·`chmod -R 777`)은 **사용자 확인 창**, 절차 형식(상태 파일 스키마·사이클 폴더 파일 종류)은 **경고만** 한다. 차단·확인·경고는 전부 `.devkit/audit.jsonl`에 기록된다(`/kit audit`로 조회).
  - ⚠ 왜 셋으로 갈랐나(2026-09-17): 전부 막던 시절 21개 프로젝트에서 차단 187건이 나왔고, 의존성 차단은 Claude가 **라이브러리 없는 설계로 몰래 우회**하게 만들었다. Trail of Bits도 hook 차단은 되돌릴 수 없는 2가지(`rm -rf`, main 직접 푸시)만 두고 안전은 OS 샌드박스에 맡긴다 — hook은 "보안 경계가 아니다"(그들의 문서).
  - 확인 창이 거절되면 **같은 효과를 내는 다른 명령으로 우회하지 않는다.** 이유와 대안을 말하고 사용자가 고르게 한다.
