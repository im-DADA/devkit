...
2. 아니면 **`planner` 에이전트를 Task로 띄워** 읽기전용 탐색과 PLAN 본문 초안을 받는다. 요청 원문과 대화에서 정해진 제약을 그대로 넘긴다. 이 단계에선 **수정 금지**.
   - planner는 **Fable·max**로 돈다(에이전트 frontmatter). 메인 세션이 직접 탐색·작성하면 세션 모델(구현용)로 계획이 짜여 단계별 모델 배분이 무의미해진다 — hook으로는 메인 세션 모델을 못 바꾼다.
...
4. planner가 반환한 본문으로 `docs/{cycle}/PLAN.md` 작성 — **파일은 메인이 쓴다**(서브에이전트는 문서 파일을 쓰지 않는다). `{폴더명}`을 채우고, 아래 형식에서 어긋난 곳만 고친다. 첫머리는 이 형태 그대로:
   ```md
   # PLAN — {제목}
