...
2. **하드 게이트** — `docs/{cycle}/behaviors.json`이 없거나 `GAP.md`가 없으면 "Gap을 먼저 수행하라(`/gap`)"고 안내하고 **여기서 중단**한다. 통과 조건은 `unproven == 0`(증거 없는 통과 주장이 0건)이다 — Match Rate 숫자가 아니다.
...
5. 사용자에게 보여주고 확인받은 뒤 **아카이빙** — `docs/{cycle}/PROGRESS.md`에 `- {date} report: 아카이빙` append 후, 사이클 폴더(PLAN·DESIGN·behaviors.json·GAP·REPORT·PROGRESS 전부)를 `docs/archive/{YYYY-MM-DD}/{slug}/`로 이동(날짜는 사이클 시작일). 상태를 `stage:"done"`, `status:"done"`으로 갱신.
