...
- **프로젝트 `AGENTS.md`의 "## 공통 규칙" 절이 있으면 Read한다**(`/kit init` 산출물). 없으면 아래 요약으로 진행한다 — 무엇을 TDD로 고정할지는 **계약 카탈로그: 멱등성·경계값·테넌트 격리·대소문자 우회·이중 처리·금액 계산**이다. **UI·마크업은 TDD 대상이 아니다** — 그건 시각 검증으로 넘긴다.
- 프로젝트의 **기존 테스트 러너/프레임워크 탐색** (Glob `*.test.*`·`*.spec.*`, `package.json` scripts). 그걸 사용 — 새 프레임워크/의존성을 **임의로** 도입하지 않는다. 필요하면 멈추고 묻는다. ⚠ 승인이 귀찮다는 이유로 러너 없이 테스트를 건너뛰거나 범위를 줄이지 말 것 — 묻지 않고 우회하는 쪽이 더 나쁘다.
