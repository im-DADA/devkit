...
- ❌ **누락** — 설계에 있는데 구현에 없음
  - ⚠ **"없음"은 "못 찾음"보다 훨씬 비싼 주장이다.** ❌를 주기 전에 **어디를 봤는지** 스스로 대답하라 — 이름이 다를 수 있고(같은 동작, 다른 함수명), 다른 층에 있을 수 있고(`instrumentation.ts`·`middleware.ts`·설정 파일·빌드 스크립트), 이미 있던 코드에 흡수됐을 수 있다. 한 번의 Grep으로 안 나온 것을 누락으로 적으면 **있는 구현을 없다고 보고**하게 된다.
  - 조회 가능한 것은 조회한다 — 테스트를 돌리면 되는 것을 코드만 읽고 "미구현"이라 하지 마라.
